...
const App = () => {
  ...
  const fetchingData = async () => {
    try {
      const res = await fetch(
        "https://diagnostic-buttery-aristosuchus.glitch.me/tasks"
      );
  ...
  const deleteData = async (id) => {
    await fetch(
      `https://diagnostic-buttery-aristosuchus.glitch.me/tasks/${id}`,
      {
        method: "DELETE",
        headers: {
  ...
  const puttingRequest = async (id, newData) => {
    fetch(`https://diagnostic-buttery-aristosuchus.glitch.me/tasks/${id}`, {
      method: "PUT",
      headers: {
        "Content-type": "application/json",
      },
